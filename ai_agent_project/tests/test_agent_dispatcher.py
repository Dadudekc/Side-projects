...
class TestAgentDispatcher(unittest.TestCase):
    ...
    @patch(
        "agents.agent_registry.AgentRegistry.list_agents", return_value=["TestAgent"]
    )
    ...
    @patch("agents.agent_registry.AgentRegistry.get_agent")
    @patch("logging.error")
    def test_dispatch_task_with_non_agentbase_instance(
        self, mock_error, mock_get_agent
    ):
        ...
        mock_error.assert_called_with(
            "Agent 'TestAgent' does not inherit from AgentBase."
        )
    ...
    def test_dispatch_task_with_exception(self, mock_error, mock_get_agent):
        ...
        mock_error.assert_called_with(
            "Error executing task for 'TestAgent': Task error"
        )
