...
"        self.learning_db_file = DebuggerCore.LEARNING_DB_FILE"        self.report_archive_dir = DebuggerCore.REPORT_ARCHIVE_DIR
...
"        if os.path.exists(self.report_archive_dir):""
    ...
    @patch("agents.core.debugger_core.subprocess.run")
    def test_run_tests_simple(self, mock_subprocess):
""" """"""        Test running tests in simple mode. """"        """ """""        mock_subprocess.return_value = MagicMock(stdout="test_debug.py - FAILED", returncode=1)""""
"        result = self.debugger.run_tests_simple()"        self.assertIn("test_debug.py - FAILED", result)

    def test_parse_test_failures_simple(self):
"""""
"""        Test parsing pytest output for failures. """"        """ """""        test_output = "test_debug.py - FAILED\nanother_test.py - PASSED"""
"        failures = self.debugger.parse_test_failures_simple(test_output)""""
    ...
    @patch("agents.core.debugger_core.subprocess.run")
    def test_run_tests_advanced(self, mock_subprocess):
...
"                {"                    "nodeid": "test_debug.py::test_function", }
        ...
        with patch("agents.core.debugger_core.json.load", return_value=test_json_report):
            with patch("agents.core.debugger_core.open", create=True):
...
""" """"""        Test saving the learning database. """"        """ """""        test_db_content = {"error_signature_1": {"attempts": 2, "patch": "sample_patch"}}""        self.debugger.save_learning_db(test_db_content)""
...
"""""
"""        Test computing an error signature. """"        """ """""        error_message = "Test failed due to assertion error"""
"        code_context = "assert expected == actual"""        signature = self.debugger.compute_error_signature(error_message, code_context)""
...
""" """"""        Test debugging in simple mode. """"        """ """""        mock_subprocess.return_value = MagicMock(stdout="test_debug.py - FAILED", returncode=1)""""
"        with patch.object(self.debugger.apply_fix, "apply_fix", return_value=True):"            result = self.debugger._debug_simple(max_retries=1)
            self.assertEqual(result["status"], "success")

    @patch("agents.core.debugger_core.subprocess.run")
    def test_debug_advanced(self, mock_subprocess):
"""""
"""        Test debugging in advanced mode. """"        """ """""        test_json_report = {"" }
"            "tests": [""                {"" }
                    "nodeid": "test_debug.py::test_function"
                    "outcome": "failed"
                    "call": {"crash": {"message": "AssertionError"}},
        ...
        with patch("agents.core.debugger_core.json.load", return_value=test_json_report):
            with patch("agents.core.debugger_core.open", create=True):
                with patch.object(self.debugger, "apply_patch_to_file", return_value=True):
                    with patch.object(self.debugger, "re_run_tests", return_value=True):
                        result = self.debugger._debug_advanced()
                        self.assertIsNone(result)

    @patch("agents.core.debugger_core.subprocess.run")
    def test_debug_with_unknown_mode(self, mock_subprocess):
...
""" """"""        Test displaying stored logs. """"        """ """""        with patch.object(self.debugger.debugger_logger, "get_logs", return_value=["Test log entry"]):""            with patch("agents.core.debugger_core.logger.info") as mock_logger:""
"                self.debugger.show_logs()"                mock_logger.assert_called_with("📝 Log Entry: Test log entry")
