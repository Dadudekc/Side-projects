...
This script automates the generation of Python module docstrings via OpenAI. It starts by checking for an OpenAI API key and appropriate directories. If these settings exist, it then defines a chat prompt template for the OpenAI request based on software architect perspective and proceeds to define functions to interact with OpenAI, extract existing docstrings, clean code, update docstrings, and progress through a specified project directory. If executed as is, it tests the insert_or_update_docstring function on a single
"""
