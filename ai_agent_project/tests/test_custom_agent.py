...
"""Test the solve_task method with valid task execution.""""
    ...
    @patch.object(
        CustomAgent, "perform_task_logic", side_effect=ValueError("Simulated error.")
    )
...
"""Test perform_task_logic returns expected result.""""
        result = self.agent.perform_task_logic(
"Sample Task", additional_info="Some info""
        )
        self.assertEqual(
result, "CustomAgent completed the task: Sample Task. Details: Some info""
        )
...
"""Test that shutdown logs properly.""""
        with self.assertLogs(level="INFO") as log:
            self.agent.shutdown()
        self.assertTrue(
            any("CustomAgent is shutting down" in msg for msg in log.output)
        )
